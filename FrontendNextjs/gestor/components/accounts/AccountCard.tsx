'use client'
import { ArrowDownRight, ArrowUpRight, Building, Edit, MoreHorizontal, Trash2 } from 'lucide-react'
				...
				<div className='space-y-3'>
					<div className='flex justify-between items-center'>
						...
						<div className='flex items-center gap-2'>
							...
							<span
								className={`font-bold text-2xl ${isPositive ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
							>
					...
					{initialBalance !== currentBalance && (
						<div className='flex justify-between items-center text-xs'>
							<span className='text-muted-foreground'>
								{t('initial')}: {formatCurrency(initialBalance, account.currency || 'USD')}
							</span>
							<span className={`font-medium ${hasGrowth ? 'text-green-600' : 'text-red-600'}`}>
								{hasGrowth ? '+' : ''}
								{formatCurrency(currentBalance - initialBalance, account.currency || 'USD').replace(/^[^\d-]+/, '')}
							</span>
						</div>
					)}
			...
			<Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>{t('deleteDescription', { name: account.name })}</DialogDescription>
					</DialogHeader>
					<DialogFooter>
