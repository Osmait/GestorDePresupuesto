...
import { Link2, Loader2, CheckCircle2, AlertTriangle, CircleHelp } from 'lucide-react'
...
export function ReconciliationModal({ open, onOpenChange }: ReconciliationModalProps) {
	...
	const result = reconcilePreview.data && 'success' in reconcilePreview.data && reconcilePreview.data.success
	...
	const handlePreview = async () => {
		...
		const response = previewResult && 'success' in previewResult && previewResult.success
			? previewResult
			: null
		if (response) {
	...
	const selectedCreateCount = allActionableItems.filter((item) => actionByTransactionId[item.extracted.id] === 'create').length
	const selectedLinkCount = allActionableItems.filter((item) => actionByTransactionId[item.extracted.id] === 'link').length
	...
	const handleApplyActions = async () => {
		...
		const actionableCount = allActionableItems.filter((item) => actionByTransactionId[item.extracted.id] !== 'ignore').length
			...
			<DialogContent className='max-w-3xl max-h-[90vh] overflow-y-auto'>
				...
				<div className='space-y-4'>
					...
					{allActionableItems.map((item) => (
						<div key={item.extracted.id} className={`rounded border p-3 flex items-start gap-3 ${failedByTransactionId[item.extracted.id] ? 'border-red-300 bg-red-50/70' : ''}`}>
							<div className='mt-0.5 min-w-28'>
											...
											<div className='flex-1'>
												...
												<div className='text-xs text-muted-foreground'>
													{item.extracted.type_transation} - {item.extracted.currency || 'DOP'} {item.extracted.amount.toFixed(2)}
												</div>
											...
											{(actionByTransactionId[item.extracted.id] || 'ignore') === 'create' && (
												<div className='mt-2 max-w-xs'>
													...
													<Select value={categoryByTransactionId[item.extracted.id] || ''} onValueChange={(value) => setCategory(item.extracted.id, value)}>
														<SelectTrigger className='h-8'>
											...
											{failedByTransactionId[item.extracted.id] && (
												<div className='mt-2 rounded border border-red-300 bg-red-100/70 px-2 py-1 text-xs text-red-700'>
													{failedByTransactionId[item.extracted.id].code}: {failedByTransactionId[item.extracted.id].message}
												</div>
											)}
										</div>
									</div>
