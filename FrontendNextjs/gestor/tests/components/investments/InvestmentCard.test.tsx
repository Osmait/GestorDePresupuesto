...
describe('InvestmentCard', () => {
	...
	it('renders investment details and positive profit correctly', () => {
		render(<InvestmentCard investment={mockInvestment} onEdit={vi.fn()} onDelete={vi.fn()} />)
		...
		expect(screen.getByText((content) => content.includes('+') && content.includes('300'))).toHaveClass(
			'text-green-600',
		)
	})
	...
	it('renders negative profit correctly', () => {
		...
		render(<InvestmentCard investment={lossInvestment} onEdit={vi.fn()} onDelete={vi.fn()} />)
	...
	it('triggers actions', async () => {
		...
		render(<InvestmentCard investment={mockInvestment} onEdit={onEdit} onDelete={onDelete} />)
