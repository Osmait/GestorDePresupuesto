...
type AccountRequest struct {
	Name           string  `json:"name" validate:"required,min=2,max=100,printascii" binding:"required"`
	Bank           string  `json:"bank" validate:"required,min=2,max=100,printascii" binding:"required"`
	InitialBalance float64 `json:"initial_balance" validate:"min=0,max=999999999.99" binding:"required"`
}
