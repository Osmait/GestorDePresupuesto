...
2. **`src/config/config.go`** - Soporte para SQLite
3. **`src/internals/platform/utils/migratios.go`** - Utilidades SQLite
4. **`src/cmd/api/db/migrations/`** - Migraciones SQLite

## Ejecutar Tests

```bash
# Todos los tests de almacenamiento
go test ./src/internals/platform/storage/postgress/test/ -v

# Todos los tests de servicios
go test ./src/internals/services/... -v

# Test específico
go test ./src/internals/platform/storage/postgress/test/ -run TestAccountRepository -v
```
